...
class ChannelDeconv2D(tf.keras.layers.Layer):
    def __init__(self, block, eps=1e-5, n_iter=5, momentum=0.1, sampling_stride=3, **kwargs):
        ...
        self.running_mean1 = tf.Variable(tf.zeros([block, 1]), trainable=False, dtype='float32')
        self.running_mean2 = tf.Variable(tf.zeros([]), trainable=False, dtype='float32')
        self.running_var = tf.Variable(tf.ones([]), trainable=False, dtype='float32')
        self.running_deconv = tf.Variable(tf.eye(block), trainable=False, dtype='float32')
    ...
    def call(self, x, training=None):
        ...
        if self.num_batches_tracked == 0:
            self.running_mean1.assign(tf.cast(mean1, self.running_mean1.dtype))

        if training:
            running_mean1 = tf.cast(self.momentum * mean1, self.running_mean1.dtype) + (1. - self.momentum) * self.running_mean1
            self.running_mean1.assign(running_mean1)
        else:
            mean1 = tf.cast(self.running_mean1, mean1.dtype)
        ...
        if c != C:
            ...
            x_s = tf.cast(x_s, self.running_mean2.dtype)
            ...
            x_tmp = tf.sqrt((tf.cast(x[:, :, :, c:], mean2.dtype) - mean2) / (var + self.eps))
            x_tmp = tf.cast(x_tmp, x1.dtype)
...
class FastDeconv2D(Conv):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding='valid', dilation_rate=1,
        ...
        if self.groups == 1:
            self.running_mean = tf.Variable(tf.zeros(self.num_features), trainable=False, dtype='float32')
            self.running_deconv = tf.Variable(tf.eye(self.num_features), trainable=False, dtype='float32')
        else:
    ...
    def call(self, x, training=None):
        ...
        if training and (not frozen):
            ...
            if self.kernel_size[0] > 1:
                # [N, L, L, C * K^2]
                X = tf.image.extract_patches(tf.cast(x, tf.float32),
